...
export default function GameCard({ game, isEditable = true }: { game: GroupedGame; isEditable?: boolean }) {
  ...
  // Cover path now comes from the API route (/api/ps1-covers/ID)
  const getCoverPath = (path: string | null) => {
    if (!path) return defaultCover;
    // If it already starts with /api/, use it directly
    if (path.startsWith('/api/')) return path;
    // Legacy fallback: encode local paths
    let cleanPath = path;
  ...
  const [imgSrc, setImgSrc] = useState(getCoverPath(game.coverPath));
            ...
            {optimisticVariants.map((variant) => (
              ...
              >
                ...
                <button
                  ...
                  title={!isEditable ? 'Somente leitura' : (variant.owned ? "Remover da Coleção" : "Adicionar à Coleção")}
                >
